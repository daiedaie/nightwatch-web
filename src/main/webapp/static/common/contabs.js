...
function menuItem() {
    ...
    // 选项卡菜单不存在
    if (flag) {
        var str = '<a href="javascript:;" class="active J_menuTab" data-id="' + dataUrl + '"><span>' + menuName + ' </span><i class="fa fa-times-circle"></i></a>';
...
function openTab(url,title, isNew){//isNew 为true时，打开一个新的选项卡；为false时，如果选项卡不存在，打开一个新的选项卡，如果已经存在，使已经存在的选项卡变为活跃状态。
	var dataUrl1 = url;
	 // 获取标识数据
  //  var dataUrl = url.indexOf("?")<=0 ? url : url.substr(0,url.indexOf("?")),
	var dataUrl = url,
        dataIndex = 111,
        menuName = title,
        flag = true;
...
//    });
    ...
    if(!isNew){
		    top.$('.J_menuTab').each(function () {
		    	var menuTabId = top.$(this).data('id');
//		        if ((menuTabId.indexOf("?")<=0 ? menuTabId : dataId.substr(0,url.indexOf("?"))) == dataUrl) {// 选项卡已存在，激活。
...
//		                	console.log(iframeId);
    ...
    if(isNew || flag){//isNew为true，打开一个新的选项卡； flag为true，选项卡不存在，打开一个新的选项卡。
	        var str = '<a href="javascript:;" class="active J_menuTab" data-id="' + dataUrl1 + '"><span>' + menuName + ' </span><i class="fa fa-times-circle"></i></a>';
	        top.$('.J_menuTab').removeClass('active');
	
	        // 添加选项卡对应的iframe
	        var str1 = '<iframe class="J_iframe" urlid="'+dataUrl+'" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl1 + '" frameborder="0" data-id="' + dataUrl1 + '" seamless></iframe>';
